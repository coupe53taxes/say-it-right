# Full Integrated Streamlit Debate Moderator App (Copy/Paste Ready)
...
# Final Summary (Post-End)
    ...
    if st.button("Let AI Decide the Winner"):
        judge_prompt = [
            {"role": "system", "content": "You're a fair judge. Decide who made the stronger case in this debate and why."},
            {"role": "user", "content": "\n\n".join([f"{entry['user']}: {entry['message']}" for entry in st.session_state.fight_history])}
        ]
        result = call_gpt(judge_prompt)
        st.subheader("🏆 AI Judgement")
        st.write(result)
